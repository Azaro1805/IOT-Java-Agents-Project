...
public class HouseMO100 extends SmartHomeAgentBehaviour {
    ...
    private void improveSchedule() {
        ...
        double bestGrade=0;
        double newGrade;
        double [] bestSched=null;
        Map<PropertyWithData, Set<Integer>> bestSchedForAllProps = new HashMap<>(propToSubsetsMap.size());
        
        for(int i=0; i<100; i++) {//100 random schedules for the props, pick the best one and save its' parameters
        		randomSchedForAllProps.clear(); //empty the previous schedule
        ...
        if (newGrade < prevGrade && flipCoin(0.8f)) { //pick the new schedule
            helper.totalPriceConsumption = newGrade;
            helper.ePeak = calculateEPeak(allScheds);
            bestSchedForAllProps.forEach((prop, ticks) ->
